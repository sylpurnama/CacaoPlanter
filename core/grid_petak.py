...
class GridBlockBuilder:
    ...
    def build_grid(self,
        ...
        while y < y1:
            col = 1
            is_offset = pattern in ("triangular", "segitiga") and row % 2 == 0
            x_start = x0 + dx_native / 2.0 if is_offset else x0
            x = x_start
            ...
            while x < x1 + dx_native:
                ...
                if field_geom.intersects(cell_geom):
                    ...
                    if clipped and not clipped.isEmpty():
                        is_inside = field_geom.contains(cell_geom)
                        status = "inside" if is_inside else "edge"
